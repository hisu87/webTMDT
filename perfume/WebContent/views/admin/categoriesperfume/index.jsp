									...
									<div class="tab-content">
										...
										<div id="home" class="container tab-pane active card"><br>
											<% if(!"".equals(request.getParameter("msg"))){ String
												...
												<% } } %>
													<% if(request.getAttribute("catPFList") !=null){ List<CatPerfume>
														catPFList = (List<CatPerfume>)request.getAttribute("catPFList");
															if(catPFList.size() > 0){
															%>
																	...
																	<table class="table table-striped table-bordered">
																		<thead>
																			<tr>
																				...
																				<th scope="col">
																					Chức năng &emsp;
																					<a href="<%=request.getContextPath()%>/admin/catperfume/add"
																						class="btn btn-success btn-md">Thêm</a>
																				</th>
																			</tr>
																		</thead>
																		<tbody>
																			<% int i=0; for(CatPerfume objCat :
																				catPFList){ i++; int id=objCat.getId();
																				%>
																				<tr>
																					<th scope="row">
																						<%=i %>
																					...
																					<td>
																						<%=objCat.getCatPerfume() %>
																					</td>
																					<td>
																						<% if(objCat.getParrent_id()
																							!=0){ CatPerfume catP=new
																							CatPerfumeDao().viewID(objCat.getParrent_id());
																							%>
																							<%=catP.getCatPerfume()%>
																								<% } else {
																									out.print(""); } %>
																					...
																					<td>
																						<a href="<%=request.getContextPath()%>/admin/catperfume/edit?id=<%=id %>"
																							title=""
																							class="btn btn-primary"><i
																								class="fa fa-edit "></i>
																							Sửa</a>
																						<a catPF=<%=id%> class="btn
																							btn-danger
																							del-catperfume"><i
																								class="fa fa-trash"
																								aria-hidden="true"></i>
