											...
											<section class="content">
												<div class="container-fluid">
													...
													<div class="row">
														<div class="col-sm-12">
															<form method="GET"
																action="<%=request.getContextPath()%>/admin/order/search">
																<input type="submit" name="search" value="Tìm kiếm"
																	class="btn btn-warning" style="float:right" />
																<input type="search" name="namePro" value=""
													...
													<table class="table table-striped table-bordered">
														...
														<tbody>
															<% int i=0; Long money=0L; for(Item product : itemList){
																i++; int quantity=product.getQuantity(); Long
																price=product.getPrice(); money=quantity * price; int
																idPer=product.getOrder().getId(); %>
																<tr class="
				<%
		      		if(product.getOrder().getStatus() == false){
		      			out.print(" table-warning"); } %>
																	...
																	<td>
																		<%=product.getProduct().getPerfumes() %>
																	</td>
																	<td>
																		<%=product.getOrder().getCustomer().getFullname()
																			%>
																	</td>
																	<td>
																		<%=product.getQuantity() %>
																				...
																				<div class="input-group">
																					<div
																						...
																						<input id="toggle-state"
																							class="toggle-state-add toggle-state-status"
																							type="checkbox"
																							data-toggle="toggle" <%
																							if(product.getOrder().getStatus()==true){
																							out.print("checked"); }else{
																							out.print("unchecked"); } %>
																						idPer = <%=idPer%>
																							>
																	...
																	<td>
																		<button idOrder=<%=product.getId() %>
																			type="button" class="btn btn-primary
																			view-product" data-toggle="modal"
										...
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												<div class="modal-header bg-info">
													<h5 class="modal-title" id="exampleModalLabel">Thông tin đơn hàng
													</h5>
